...
#line 2 "counter_people.c"
#line 1 "counter_people.h"
...
void channel_filtering(unsigned char *channels, 
                        unsigned int size,
                        struct people *atv_channels,
                        unsigned int *atv_channels_size,
                        unsigned char filtered_width_threshold);

unsigned int channel_analyze(struct people *channels,
                            unsigned int size,
...
static double _us_sensors[((8) * (3))];
static int _us_max_heights[((8) * (3))];
static int _us_keep_counters[((8) * (3))] = {35};

static double _pir_sensors[((8) * (3))];
static double _pir_means[((8) * (3))];
static double _pir_sums[((8) * (3))]; 
static double _pir_filter_buffers[((8) * (3))][2];
static unsigned char _pir_filter_idxs[((8) * (3))];

static int iii = 0;
static int ii = 0;
static int _iiold = 0;
static double dT_pos[(((8) / 2) * (3))][2];
static double dT_neg[(((8) / 2) * (3))][2];
static int _iLastDetectioni = 0;

static double dThresh1 = 0;
static double dThresh2 = 0;

static double dT[((((8) / 2) * (3)) * 2)];
static double dTold[((((8) / 2) * (3)) * 2)];
static double dTmax[(((8) / 2) * (3))][2];
static int iTmax_time[(((8) / 2) * (3))][2];

static unsigned char bFlag1[(((8) / 2) * (3))][2];
static unsigned char _bState = 0;
static unsigned char _bLastState = 0;
static unsigned char bLane_State[(((8) / 2) * (3))][50];
static int iDuration[(((8) / 2) * (3))][50];
static unsigned char bLane_Tmax[(((8) / 2) * (3))][50];

static unsigned char bMode = 0;
static unsigned char bCalibrationCnt = 0;

static int iHighDuration[(((8) / 2) * (3))][2];
...
static int Calibration(void)
{
    ...
    if (bCalibrationCnt == 60 - 1) {
        for (i = 0; i < _pir_number; i++) {
            _pir_means[i] = _pir_sums[i] / 60;
            if (_pir_means[i] < 0 || _pir_means[i] > 35) {
                printk("e", "pir_means(%d)=%f invalid\r\n", i, _pir_means[i]);
                return -1;
            }
        }
         
        for (i = 0; i < _board_number; i++) {
            printk("d", "Calibration(%d),%f,%f,%f,%f,%f,%f,%f,%f\r\n",
                    i, 
...
static int us_get_height(unsigned char begin, unsigned char width)
{
    int max_height = 0;
    unsigned char us;
    unsigned char board_idx = (begin / ((8) / 2));
...
static void HistoryShift(void)
{
    ...
    for (lane = 0; lane < _pir_lane; lane++) {
        for (i = 0; i < ii; i++) {
            bLane_State[lane][i] = bLane_State[lane][50 - ii + i];
            iDuration[lane][i] = iDuration[lane][50 - ii + i];
            bLane_Tmax[lane][i] = bLane_Tmax[lane][50 - ii + i];
        }
        for (i = ii; i < 50; i++) {
    ...
    for (lane = 0; lane < _pir_lane; lane++) {
        ...
        if (bFlag1[lane][0] == PASSED && bFlag1[lane][1] == PASSED) {
            ...
            if (index1 >= index2) {
                ...
                printk("d", ">error,sample=%d,sensor=%d,index1=%d<index2=%d\r\n", 
                                    count, (lane * 2 + row + 1), index1, index2);
#line 396 "counter_people.c"
            if (dT_Tpos >= 0 && dT_Tneg > 0) { 
                bLane_State[lane][ii] = 2;
                
            } else if (dT_Tpos <= 0 && dT_Tneg < 0) { 
...
static int pir_lane_analyze(unsigned int count,
                            ...
                            unsigned char people_max_count)
{
    struct people atv_channels[(((8) / 2) * (3))];
    unsigned int atv_channels_size = 0, people_count;
    unsigned char chan[(((8) / 2) * (3))];
    ...
    for (lane = ActiveLaneCnt = 0; lane < _pir_lane; lane++) {
        chan[lane] = Search_LaneStateMax(lane, ii - 12, ii);
    }
    ...
    if (people_count > 0) {
        ...
        for (lane = 0; lane < _pir_lane; lane++) {
            if (iDuration[lane][ii - 12] > iMaxLaneDuration) {
                iMaxLaneDuration = iDuration[lane][ii - 12];
                ActiveLaneCnt = lane;
            }
        }
        if (count == 955)
            printk("d", "count=%d,iMaxLaneDuration=%d, "                             "iii=%d,_iLastDetectioni=%d,"                             "iii - _iLastDetectioni=%d,"                             "_bState=%d,_bLastState=%d\n",



                            count, iMaxLaneDuration, 
                            iii+60, _iLastDetectioni + 60, 
        ...
        if (iMaxLaneDuration > 16) {
            ...
            if (iMaxLaneDuration > iii - _iLastDetectioni) {
                if (_bLastState == _bState || _bLastState == 3) {
                    ...
                    printk("d", "MaxDuration,sample=%d,lane=%d,duration=%d\r\n",
...
int counter_people_init(double t1, double t2, unsigned char board_number)
{
    ...
    memset(_us_keep_counters, 35, sizeof(_us_keep_counters));    
        ...
        for (j = 0; j < ((8) + (8)); j++) {
            ...
            if (j < (8)) {
                ...
                if ((value & 0x0100) != 0x0100) {
                    ...
                    _pir_sensors[pi] = 23;
                    printk("e", "process(%d,%d)=0x%x invalid\r\n", i,j,value);
                    return -1;
                }
                pi++;
            } else {
                _us_sensors[ui] = 2.54 * value / 2;
                if (_us_sensors[ui] > 190.0)
                    _us_sensors[ui] = 190.0;
                ui++;
            }
        }
    }
