...
static volatile uint8_t _min_buffer[512];
static volatile uint8_t _max_buffer[2][2048];
...
static UART_T *address_array[] = {((UART_T *) ((( uint32_t)0x40000000) + 0x50000)), ((UART_T *) ((( uint32_t)0x40100000) + 0x50000)), ((UART_T *) ((( uint32_t)0x40100000) + 0x54000))};
...
static void _UART_INT_HANDLE(E_UART_PORT port, uint32_t u32IntStatus)
{
    ...
    if (u32IntStatus & DRVUART_RDAINT) {
        ...
        while (address->ISR.RDA_IF == 1) {
            ...
            if (DrvUART_Read(port, bInChar, 1) == 0) {
...
{
    return DrvUART_Write(port, pu8TxBuf, u32WriteBytes) == 0 ? 0 : -1;
}
...
int uart_write_uint32(E_UART_PORT port, uint32_t integer)
{
    ...
    memmove(buffer, (uint8_t *) &integer, 4);
    return uart_write(port, buffer, sizeof(buffer));
}
