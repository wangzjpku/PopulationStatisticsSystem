...
{
 uint8_t i;
 int32_t ADC_calibration;                                    

  DrvADC_StartConvert();                                               
  while(!DrvADC_IsConversionDone())                                    
  {
  }
  ADC_calibration = DrvADC_GetConversionData(ADC_channel_number);   
  DrvADC_StopConvert();                                                
...
void SetADChannel( uint32_t channel )
{
    ...
    dout_temp = ((GPIO_T *) (((( uint32_t)0x50000000) + 0x4000) + 0x0080))->DOUT & 0xFFFF3F3F;
    dout_temp |=  ( ( ( channel  & 0x01 ) << 7 ) | ( ( (channel >> 1) & 0x1 ) << 6 ) | (((channel >> 2) & 0x03 ) << 14) );           

    ((GPIO_T *) (((( uint32_t)0x50000000) + 0x4000) + 0x0080))->DOUT = dout_temp;
}
...
{
    _sensor_set_default(sample, _pir_default);
    _sensor_set_default(sample + 16, _us_default);
}
...
int sensor_read(uint8_t *sample, uint32_t max_size, enum board_type type)
{
    ...
    pir_read((sl_sample));
    _sensor_set_default(((sl_sample) + 16), _us_default);
     
    if (slave_us_is_enabled(type)) {
        (*((volatile uint32_t *) (((((( uint32_t)0x50000000) + 0x4000) + 0x200)+(0x40*0)) + (0x4*1)))) = (*((volatile uint32_t *) (((((( uint32_t)0x50000000) + 0x4000) + 0x200)+(0x40*2)) + (0x4*11)))) = 0;
        if ((++_us_r_counter) <= 4)
            us_read = 1;
    } else {
        (*((volatile uint32_t *) (((((( uint32_t)0x50000000) + 0x4000) + 0x200)+(0x40*0)) + (0x4*1)))) = (*((volatile uint32_t *) (((((( uint32_t)0x50000000) + 0x4000) + 0x200)+(0x40*2)) + (0x4*11)))) = 1;
        _us_r_counter = 0;
    }
    if (us_read) {
        ultrasonic_read_next(((sl_sample) + 16));
        


 
        sl_sample[17] = _us_r_counter;
    } else
        DrvSYS_Delay((12.5 * 1000));
...
static int _read_one_board(uint8_t *sample)
{
    ...
    if ((++_us_r_counter) > 8)
        _us_r_counter = 1;
    ultrasonic_read_next(((sample) + 16));
    _set_default(((sample) + 32));
    _set_default(((sample) + 64));
    return 0;
}
...
static int _read_two_left_board(uint8_t *sample)
{
    ...
    int rc;
    uint8_t *sl_sample = (sample);
    uint8_t *master_sample = ((sample) + 32);
    uint8_t slave_left_enabled = _us_r_counter < 5 ? 1 : 0;
    uint32_t sample_size = 0;
 
     
    if (_us_r_counter < 5)
        slave_us_enable(UART_PORT1);

     
    rc = slave_read_sample(UART_PORT1, sl_sample, 32, &sample_size);
    if (rc) {
        ...
        if (sample_size == (1 + (32 >> 1))) {
            sensor_uncompress(sl_sample, sample_size, master_sample);
            memmove(sl_sample, master_sample, 32);
        }
    ...
    pir_read((master_sample));
    if (slave_left_enabled) {
        _sensor_set_default(((master_sample) + 16), _us_default);
        DrvSYS_Delay((12.5 * 1000));
    } else {
        ultrasonic_read_next(((master_sample) + 16));
        if ((++_us_r_counter) >= 9)
            _us_r_counter = 1;
    }
