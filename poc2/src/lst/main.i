...
static void _do_slave(uint32_t timestamp)
{
    ...
    sensor_read(_sample, 32, system_get_board_type(_dip));
    if (use_compress && sensor_compress(_sample, 
                                        32, 
                                        _sample + 32, 
                                        &compressed_size) == 0) {
        { uart_write_uint16(UART_PORT2, 0xa3a3); uart_write_byte(UART_PORT2, compressed_size); uart_write(UART_PORT2, _sample + 32, compressed_size); uart_write_byte(UART_PORT2, 0x00); };


    } else {
        { uart_write_uint16(UART_PORT2, 0x6868); uart_write_byte(UART_PORT2, 32); uart_write(UART_PORT2, _sample, 32); uart_write_byte(UART_PORT2, 0x00); };
    }
}
...
static int _do_pc_frame(uint8_t *sample, uint32_t max_size)
{
    ...
    if (((sample[0] << 8) + sample[1] == 0x7ab7)) {
        ...
        DrvSYS_Delay(5 * 1000);
        printk("d", "pc, board_number=%D, mode=%d, t1=%f, t2=%f\r\n", 
                        bnumber, work_mode, t1, t2);

        if ((work_mode == MODE_LIVE || work_mode == MODE_FILE || work_mode == MODE_COLLECT) && (t1 > t2 && t1 > 20.0 && t1 < 36.0 && t2 > 20.0 && t2 < 36.0)) {
            _initialize(bnumber, work_mode, t1, t2);
        }
    } else if (((sample[0] << 8) + sample[1] == 0x0815))
...
static int _do_counter(uint8_t *sample, uint32_t size, uint32_t timestamp)
{
    ...
    if (_sample_count % 1000 == 0) {
        ...
        printk("d", "counting=%d, timestamp=%d, "                         "uart_overflow_counters=%d,%d,%d,version=%s\r\n", 
    ...
    for (i = 0; i < people_count; i++) {
        ...
        if (_peoples[i].direction == 1)
            ...
            _people_in_count++;
        printk("d", "People,Sample=%d,%s=%d,height=%d,begin=%d,width=%d\r\n", 
                _sample_count,
...
static void _do_master(uint32_t timestamp)
{
    ...
    if (_work_mode == MODE_LIVE) {
        sensor_read_all(_sample, 3 * 32, _board_number);
        memmove(&_sample[3 * 32], (uint8_t *) &timestamp, 4);
        _do_counter(_sample, 3 * 32 + 4, timestamp);
        pc_write_sample_frame(_sample, 3 * 32 + 4);
    } else if (_work_mode == MODE_FILE) {
        if (rc == 0 && ((_sample[0] << 8) + _sample[1] == 0x6868)) {
        ...
        printk("e", "system mode=%d unknown\r\n", _work_mode);
...
static void _do(void)
{
    ...
    if (_board_number == ONE_NUMBER || (system_get_board_type(_dip) == MASTER_BOARD))
        _do_master(now);
    else {
