...
{
    if (uart_read_try(UART_PORT0, sample, 3) == 0) {
        if (((sample[0] << 8) + sample[1] == 0x6868)
            || ((sample[0] << 8) + sample[1] == 0x7ab7)
            || ((sample[0] << 8) + sample[1] == 0x0815)) {            
            
            if (uart_read_oversize(UART_PORT0) >= 3 + sample[2] + 1) {
                uart_read(UART_PORT0, sample, 3 + sample[2] + 1);
                return sample[3+sample[2]] == 0x00 ? 0 : -1;
            }
        } else
            uart_read(UART_PORT0, sample, 1);
    }
    return -1;
}
...
void pc_write_sample_frame(uint8_t *sample, uint8_t size)
{
    uart_write_uint16(UART_PORT0, 0x6868);;
    uart_write_byte(UART_PORT0, size);;
    uart_write(UART_PORT0, sample, size);;
    uart_write_byte(UART_PORT0, 0x00);;
}

void pc_write_sample_ack_frame(uint32_t timestamp)
{
    uart_write_uint16(UART_PORT0, 0x6869);;
    uart_write_byte(UART_PORT0, sizeof(timestamp));;
    uart_write_uint32(UART_PORT0, timestamp);;
    uart_write_byte(UART_PORT0, 0x00);;
}

void pc_write_people_frame(uint32_t count, struct people *people)
{
    uart_write_uint16(UART_PORT0, 0x6a2e);;
    uart_write_byte(UART_PORT0, 6);;
    uart_write_uint32(UART_PORT0, count);;
    uart_write_byte(UART_PORT0, people ->direction);;
    uart_write_byte(UART_PORT0, people ->height);;
    uart_write_byte(UART_PORT0, 0x00);;
}

void pc_write_log_frame(uint8_t *buffer, uint8_t size)
{
    uart_write_uint16(UART_PORT0, 0x3f34);;
    uart_write_byte(UART_PORT0, size);;
    uart_write(UART_PORT0, buffer, size);;
    uart_write_byte(UART_PORT0, 0x00);;
}
                            ...
                            double t2)
{
    ...
    uart_write_uint16(UART_PORT0, 0x0815);;
    uart_write_byte(UART_PORT0, (uint8_t) size);;
    uart_write(UART_PORT0, buffer, (uint32_t) size);;
    uart_write_byte(UART_PORT0, 0x00);;
}
...
void pc_uart_test(void)
{
    ...
    if (uart_read(UART_PORT0, buffer, 1) == 0)
        uart_write(UART_PORT0, buffer, 1);;
}
